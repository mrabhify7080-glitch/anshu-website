/* ==========================================================================
   PRIYA AI CHAT ASSISTANT - GLOBAL JAVASCRIPT & CONVERSATION ENGINE
   ========================================================================== */

(function () {
    // Ensure CSS is loaded
    if (!document.getElementById('priya-assistant-css')) {
        const link = document.createElement('link');
        link.id = 'priya-assistant-css';
        link.rel = 'stylesheet';
        link.href = 'css/priya-assistant.css';
    ...
    function initPriyaWidget() {
        if (document.getElementById('priya-widget-container')) return;

        const widgetHTML = `
            <div id="priya-widget-container">
                <!-- Floating Tooltip Bubble -->
                <div id="priya-tooltip">
                    <span class="priya-tooltip-close" onclick="document.getElementById('priya-tooltip').style.display='none'">&times;</span>
                    <div>
                        <strong>Namaste!</strong> I am Priya, Anshu Property AI Assistant. Are you looking to buy or sell property?
                    </div>
                </div>

                <!-- Floating Gold Trigger Button -->
                <button id="priya-trigger-btn" onclick="togglePriyaChat()" aria-label="Chat with Priya AI Assistant">
                    <img src="assets/ai_female_assistant.jpg" alt="Priya AI Assistant" class="priya-trigger-img">
                    <span class="priya-online-dot"></span>
                </button>

                <!-- Chat Box Window -->
                <div id="priya-chat-box" class="priya-hidden">
                    <!-- Navy Header -->
                    <div class="priya-header">
                        <div class="priya-header-left">
                            <div class="priya-header-avatar">
                                <img src="assets/ai_female_assistant.jpg" alt="Priya AI Assistant">
                            </div>
                            <div class="priya-header-info">
                                <h4>Priya <span style="font-size:0.68rem; background:rgba(200,155,60,0.25); color:#C89B3C; padding:2px 8px; border-radius:8px;">AI Support</span></h4>
                                <div class="priya-status-indicator">
                                    <span class="priya-status-dot"></span> Online | Anshu Property
                                </div>
                            </div>
                        </div>
                        <button class="priya-close-btn" onclick="togglePriyaChat()">&times;</button>
                    </div>

                    <!-- Messages Body (Ivory Background) -->
                    <div class="priya-messages-body" id="priya-msg-container">
                        <!-- Initial Greeting -->
                        <div class="priya-msg-row priya-msg-left">
                            <div class="priya-bubble">
                                Namaste! 🙏 Welcome to <strong>Anshu Property</strong>.<br><br>
                                I am <strong>Priya</strong>, your virtual real estate guide. How can I help you today?
                            </div>
                            <span class="priya-msg-time">Just now</span>
                        </div>
                    </div>

                    <!-- Input Footer -->
                    <div class="priya-footer">
                        <input type="text" id="priya-input" placeholder="Type your answer here..." onkeypress="handlePriyaKeyPress(event)">
                        <button id="priya-send-btn" onclick="sendPriyaUserMessage()">
    ...
    // Toggle Chat Window
    window.togglePriyaChat = function () {
        const box = document.getElementById('priya-chat-box');
        const tooltip = document.getElementById('priya-tooltip');
        if (tooltip) tooltip.style.display = 'none';

        if (box.classList.contains('priya-hidden')) {
            box.classList.remove('priya-hidden');
            document.getElementById('priya-input').focus();
        } else {
            box.classList.add('priya-hidden');
        }
    };
    ...
    window.handlePriyaChoice = function (choice) {
        if (choice === 'buy') {
            state.mode = 'buy';
    ...
    window.handlePriyaKeyPress = function (e) {
        if (e.key === 'Enter') {
            sendPriyaUserMessage();
        }
    };

    window.sendPriyaUserMessage = function () {
        const input = document.getElementById('priya-input');
    ...
    function processUserText(text) {
        // If state is waiting for custom text input (name/phone or custom seller steps)
        if (state.mode === 'buy') {
            if (state.step === 5) {
        ...
        // Generic query / fallback with Claude-style NLP response logic
    ...
    function appendUserMsg(text) {
        const container = document.getElementById('priya-msg-container');
        const time = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        const html = `
            <div class="priya-msg-row priya-msg-right">
                <div class="priya-bubble">${escapeHTML(text)}</div>
                <span class="priya-msg-time">${time}</span>
            </div>
        `;
    ...
    function appendBotMsg(htmlContent) {
        ...
        const html = `
            <div class="priya-msg-row priya-msg-left">
                <div class="priya-bubble">${htmlContent}</div>
                <span class="priya-msg-time">${time}</span>
            </div>
        `;
        container.insertAdjacentHTML('beforeend', html);
    ...
    function appendBotMsgWithChips(questionText, chipsArray) {
        let chipsHTML = `<div class="priya-options-grid">`;
        chipsArray.forEach(chip => {
            chipsHTML += `
                <button class="priya-opt-btn" onclick="selectChipValue('${chip.value}', '${chip.label}')">
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initPriyaWidget);
    } else {
        initPriyaWidget();
    }
})();
