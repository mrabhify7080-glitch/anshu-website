...
def update_file(filepath):
    ...
    if 'ai-assistant-wrapper' in content:
    ...
    if '</style>' in content:
        ...
        content = content.replace('</head>', f'<style>{ai_css}</style>\n</head>', 1)

    # Insert HTML/JS before </body>
    if '</body>' in content:
        content = content.replace('</body>', ai_html_js + '\n</body>', 1)
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Successfully updated: {filepath}")

update_file(r'd:\ansu\website\landing-page.html')
update_file(r'd:\ansu\website\index.html')
update_file(r'd:\ansu\website\AnshuProperties.com.html')
